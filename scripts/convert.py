...
- ✅ PHÂN QUYỀN ADMIN:
    + CHỈ email "hoanginvest@gmail.com" → SUPER ADMIN: full quyền
    + TẤT CẢ admin khác → ADMIN THƯỜNG
- ✅ Tổng User KHÔNG tính admin
- ✅ Nút đổi tên hiển thị (user + admin đều có)
- ✅ Quản lý ngày hết hạn ngay trên giao diện admin
- ✅ Nút chỉnh hạn sử dụng = BIỂU TƯỢNG LỊCH (icon only)
- ✅ MỚI: Header LỚN + TikTok info bar
- ✅ MỚI: Chế độ thường hiện Zalo + TikTok; Chế độ luyện tập chỉ hiện TikTok
- ✅ MỚI: PC hover nút TikTok → hiện card info TikTok
...
- ✅ FIX MỚI: BỎ HOÀN TOÀN chấm đen khi nhấn nút trên PC
- ✅ FIX MỚI: Icon nút "Tập trung học" = QUẢ CHUÔNG (fa-bell / fa-bell-slash)
- ✅ FIX MỚI: TẮT chuông = NỀN XÁM, BẬT chuông = NỀN XANH
...
print(f"   🚫 FIX: Bỏ HOÀN TOÀN chấm đen khi nhấn nút trên PC")
print(f"   🔔 FIX: Icon nút 'Tập trung học' = QUẢ CHUÔNG")
print(f"   🎨 FIX: TẮT chuông = NỀN XÁM · BẬT chuông = NỀN XANH")
...
<style>
/* ============================================================
   RESET TOÀN CỤC — KHỬ HOÀN TOÀN CHẤM ĐEN / OUTLINE / TAP-HIGHLIGHT
   ============================================================ */
*,
*::before,
*::after {
...
/* FAB: khi nhấn KHÔNG dùng shadow đậm → tránh chấm đen ở góc */
.fab-btn:active,
.fab-btn:focus,
...
.fab-main::-moz-focus-inner { border: 0 !important; padding: 0 !important; }

/* Chip select không có viền đen khi focus */
.chip:focus,
.chip:focus-visible,
.chip:active,
.pf-chip:focus,
...
.chip.has-value:focus,
.chip.has-value:focus-visible,
.pf-chip.has-value:focus,
.pf-chip.has-value:focus-visible {
...
.logo-text .title{font-size:2rem;font-weight:900;letter-spacing:-.02em;line-height:1.15;color:var(--text)}
.logo-text .subtitle{font-size:.95rem;color:var(--text-3);font-weight:700;margin-top:3px;letter-spacing:.01em;padding-left:1.1rem}
...
[data-theme="dark"] .tiktok-bar{background:linear-gradient(135deg, rgba(254,44,85,.1), rgba(37,244,238,.1))}
.tiktok-bar:hover{border-color:var(--text-2);box-shadow:0 4px 12px rgba(0,0,0,.08);transform:translateY(-1px)}
.tiktok-bar-avatar{
    width:32px;height:32px;border-radius:50%;object-fit:cover;flex-shrink:0;
    border:2px solid var(--border);background:linear-gradient(135deg,#fe2c55,#25f4ee);
}
...
.tiktok-bar-link{
    ...
    white-space:nowrap;transition:.15s;flex-shrink:0;box-shadow:0 2px 8px rgba(0,0,0,.2);
}
...
.user-details{
    ...
    margin-bottom:.5rem;display:flex;flex-direction:column;gap:.6rem;
}
...
.btn-login-header{
    ...
    transition:.15s;box-shadow:0 4px 12px rgba(37,99,235,.3);white-space:nowrap;
}
...
.chip{
    ...
    cursor:pointer;transition:.15s;min-width:0;box-shadow:var(--shadow-sm);
    -webkit-appearance:none;text-align:left;position:relative;overflow:hidden;
}
...
.zalo-btn{
    ...
    white-space:nowrap;border:2px solid #fff;overflow:hidden;position:relative;
}
...
@keyframes tiktokPulse{0%{transform:scale(1);opacity:.35}50%{transform:scale(1.1);opacity:0}100%{transform:scale(1);opacity:0}}
...
.fab-btn.active{background:var(--primary);color:#fff;border-color:var(--primary);box-shadow:0 4px 12px rgba(37,99,235,.25)}
.fab-btn.active:hover,.fab-btn.active:active,.fab-btn.active:focus,.fab-btn.active:focus-visible{
    background:var(--primary-dark);color:#fff;
    box-shadow:0 4px 12px rgba(37,99,235,.25) !important;outline:none !important;
}
.fab-btn::after{
...
.fab-sub{
    ...
    pointer-events:none !important;transition:opacity .2s, transform .25s;
}
.fab-group.open .fab-sub{
    opacity:1;transform:translateY(0) scale(1);pointer-events:auto !important;
}
...
/* ============================================================
   NÚT "TẬP TRUNG HỌC" (QUẢ CHUÔNG)
   - TẮT chuông (đang hiện Zalo/TikTok) → NỀN XÁM (mặc định)
   - BẬT chuông  (đang ẩn Zalo/TikTok)  → NỀN XANH
   ============================================================ */
.fab-focus { outline: none !important; }

/* TẮT chuông → nền xám (giống FAB chưa active) */
.fab-focus:not(.active) {
    background: var(--surface) !important;
    color: var(--text-2) !important;
    border-color: var(--surface) !important;
    box-shadow: var(--shadow-fab) !important;
}
.fab-focus:not(.active):hover,
.fab-focus:not(.active):active,
.fab-focus:not(.active):focus,
.fab-focus:not(.active):focus-visible {
...
/* BẬT chuông → nền xanh (giống FAB active) */
.fab-focus.active {
...
.fab-focus.active:focus,
.fab-focus.active:focus-visible {
...
/* Dark mode */
[data-theme="dark"] .fab-focus:not(.active) {
    background: var(--surface) !important;
    color: var(--text-2) !important;
    border-color: var(--surface) !important;
}
[data-theme="dark"] .fab-focus:not(.active):hover,
[data-theme="dark"] .fab-focus:not(.active):active {
    background: var(--surface-2) !important;
    color: #93c5fd !important;
    border-color: #3b82f6 !important;
}
[data-theme="dark"] .fab-focus.active {
    background: #3b82f6 !important;
    color: #fff !important;
    border-color: #3b82f6 !important;
}
[data-theme="dark"] .fab-focus.active:hover,
[data-theme="dark"] .fab-focus.active:active {
...
.demo-banner-btn{
    ...
    transition:.15s;display:flex;align-items:center;gap:.35rem;white-space:nowrap;
}
...
.expiry-banner.urgent{
    ...
    border-color:#dc2626;animation:pulseUrgent 2s infinite;
}
...
.toggle-check-btn{
    ...
    background:var(--surface-2);color:var(--text-2);
    cursor:pointer;display:inline-flex;align-items:center;justify-content:center;
    font-size:.8rem;transition:.15s;flex-shrink:0;border:1px solid var(--border);
}
...
.practice-full-header .pf-counter{
    ...
    background:var(--surface-2);padding:.35rem .75rem;border-radius:50px;white-space:nowrap;
}
...
.pf-chip{
    ...
    cursor:pointer;transition:.15s;min-width:0;position:relative;overflow:hidden;
}
...
.pf-result-count{
    ...
    color:var(--text-2);font-size:.75rem;font-weight:600;width:fit-content;
}
...
.practice-full-vi{
    ...
    text-align:center;line-height:1.45;padding:1.25rem 1rem;
    background:linear-gradient(135deg, var(--surface-2), rgba(37,99,235,.06));
...
.practice-full-input{
    ...
    outline:none;transition:.15s;text-align:center;letter-spacing:.05em;
    -webkit-appearance:none;
}
...
.char-slot.wrong{
    ...
    animation:shakeWrong .3s;box-shadow:0 2px 8px rgba(220,38,38,.35);
...
.char-slot.ghost{
    color:var(--text);opacity:.12;font-weight:400;background:transparent;
    user-select:none;pointer-events:none;filter:blur(0.3px);
}
[data-theme="dark"] .char-slot.ghost{color:var(--text);opacity:.15}
.char-slot.extra{
    color:#fff;background:var(--amber);
    box-shadow:0 2px 8px rgba(245,158,11,.35);cursor:pointer;transition:.15s;
}
...
.char-slot.ghost-missing{
    ...
    border:1px dashed var(--border);cursor:pointer;font-size:1.2rem;font-weight:400;
}
.char-slot.ghost-missing:hover{
    opacity:.9;border-color:var(--primary);color:var(--primary);transform:scale(1.1);
}
...
.answer-phrase-btn{
    ...
    padding:.5rem .9rem;border-radius:12px;border:2px solid var(--border);
...
@media(hover:hover) and (pointer:fine){
    .answer-phrase-btn:hover{
        transform:scale(1.35);background:var(--primary);color:#fff;
        border-color:var(--primary);box-shadow:0 8px 24px rgba(37,99,235,.4);z-index:10;
    }
}
...
.pf-nav-btn{
    ...
    transition:.15s;display:inline-flex;align-items:center;justify-content:center;gap:.5rem;
}
...
.user-row .u-role.super{
    ...
    color:#fff;box-shadow:0 2px 6px rgba(245,158,11,.4);
}
...
.u-btn.expiry{
    background:rgba(245,158,11,.1);color:#d97706;border-color:rgba(245,158,11,.4);
}
.u-btn.expiry:hover{
    background:var(--amber);color:#fff;border-color:var(--amber);transform:scale(1.08);
}
.u-btn.expiry:active{transform:scale(.95)}
[data-theme="dark"] .u-btn.expiry{
    background:rgba(245,158,11,.2);color:#fcd34d;border-color:rgba(245,158,11,.5);
}
[data-theme="dark"] .u-btn.expiry:hover{background:var(--amber);color:#fff}
.u-btn.expiry.urgent{
    background:rgba(220,38,38,.15);color:var(--danger);
    border-color:rgba(220,38,38,.5);animation:expiryPulse 2s infinite;
}
.u-btn.expiry.urgent:hover{background:var(--danger);color:#fff;border-color:var(--danger)}
[data-theme="dark"] .u-btn.expiry.urgent{
    background:rgba(220,38,38,.3);color:#fca5a5;border-color:rgba(220,38,38,.6);
}
...
.form-group input,.form-group select{
    ...
    border:1.5px solid var(--border);background:var(--surface);
    color:var(--text);font-size:.85rem;outline:none;transition:.15s;
}
...
.quick-expiry-btn{
    ...
    cursor:pointer;transition:.15s;
    display:flex;flex-direction:column;align-items:center;gap:.2rem;white-space:nowrap;
}
...
.import-info{
    margin-top:1rem;padding:.65rem .85rem;border-radius:10px;
    background:var(--primary-light);color:var(--primary-dark);
...
@media(max-width:768px){
    ...
    .floating-left-group{left:16px;bottom:calc(16px + env(safe-area-inset-bottom));gap:.45rem}
    ...
    .tiktok-float-btn{padding:0;border-radius:50%;width:48px;height:48px;justify-content:center;gap:0}
...
<div class="fab-group" id="fabGroup" style="display:none">
    ...
    <!-- ✅ NÚT "TẬP TRUNG HỌC": TẮT chuông = XÁM · BẬT chuông = XANH -->
...
function initDisplayState() {
    ...
    if (toggleFocusBtn) {
        ...
        /* ✅ Khử focus mặc định khi nhấn chuột (tránh chấm đen trên PC) */
        toggleFocusBtn.addEventListener('mousedown', function(e) {
            e.preventDefault();
            this.blur();
        });
    }
...
/* ✅ Cập nhật icon cho nút "Tập trung học"
   - TẮT chuông → icon fa-bell, nền XÁM (do class .active bị xóa)
   - BẬT chuông → icon fa-bell-slash, nền XANH (do class .active được thêm) */
...
print(f"🚫 FIX: Ẩn search bar + filters của header khi mở modal luyện tập")
print(f"🚫 FIX: Modal set top 1 lần theo chiều cao header")
print(f"🚫 FIX: Bỏ HOÀN TOÀN chấm đen khi nhấn nút trên PC")
print(f"🔔 FIX: Icon nút 'Tập trung học' = QUẢ CHUÔNG (fa-bell / fa-bell-slash)")
print(f"🎨 FIX: TẮT chuông = NỀN XÁM · BẬT chuông = NỀN XANH")
print(f"🎯 Nút FAB 'Tập trung học': ẩn/hiện Zalo+TikTok+TikTokBar")
