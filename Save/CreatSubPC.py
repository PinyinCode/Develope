...
def get_ffmpeg_path():
    """Tự động tìm đường dẫn thư mục bin của FFmpeg (hỗ trợ cả khi cài đặt bằng setup.exe)"""
...
def extract_video_id(url):
    """Trích xuất Video ID từ đường dẫn YouTube để làm tên file chuẩn trên GitHub"""
...
class SubtitleApp:
    def __init__(self, root):
        ...
        # --- CẤU HÌNH STYLE: IOS DARK MODE MINIMAL ---
        self.style = ttk.Style()
        self.style.theme_use('clam')
        
        # Bảng màu iOS Dark Mode chuẩn
        BG_COLOR = "#000000"          # Nền chính đen tuyền
        CARD_BG = "#121214"           # Nền Card dạng khối bo nhẹ giống iOS Widget
        CARD_ELEMENT = "#1C1C1E"      # Nền ô nhập liệu / phần tử con
        TEXT_PRIMARY = "#FFFFFF"      # Trắng sáng
        TEXT_SECONDARY = "#8E8E93"    # Xám chữ phụ (iOS Gray)
        
        self.root.configure(bg=BG_COLOR)
        
        # Cấu hình style widget chung
        ...
        # --- CARD 2: THƯ MỤC LƯU & TÙY CHỌN GITHUB ---
            ...
            if self.upload_gh_var.get():
                ...
                if not token:
                    token = simpledialog.askstring(
                        "Nhập Token GitHub", 
                        "Bạn đã chọn chế độ đồng bộ GitHub.\nVui lòng nhập GitHub Personal Access Token (PAT) để tiếp tục:", 
                        parent=self.root
                    )
                    if not token or not token.strip():
                        self.log("Đã hủy quá trình vì không cung cấp Token.")
                        self.upload_gh_var.set(False)
                        return
                    config['gh_pat_token'] = token.strip()
    ...
    def upload_to_github_gist(self, file_path, video_id):
        ...
        try:
            ...
            # Bước 1: Kiểm tra xem Gist chứa file này đã tồn tại trên tài khoản hay chưa
            ...
            # Bước 2: Nếu đã tồn tại -> Ghi đè (PATCH). Nếu chưa -> Tạo mới (POST)
            if existing_gist_id:
            ...
            self.log(f"✅ Đã đồng bộ thành công lên GitHub Gist (Đã ghi đè nếu trùng)!\n🔗 Raw URL: {raw_url}")
            
        except urllib.error.HTTPError as e:
            if e.code == 401:
                self.log("❌ Lỗi: Token GitHub không hợp lệ hoặc đã hết hạn. Hệ thống đã xóa Token cũ. Vui lòng thử lại sau.")
